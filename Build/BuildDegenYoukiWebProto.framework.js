...
var ASM_CONSTS = {
  5725024: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 5725085: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 5725149: function() {return Module.webglContextAttributes.powerPreference;},  
 5725207: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 5725262: function($0) {performance.now = function() { return $0; };},  
 5725310: function($0) {performance.now = function() { return $0; };},  
 5725358: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _InitializeEthProvider() {
      if (typeof window.ethereum !== 'undefined') {
        console.log("Initializing Web3.js provider...");
  
        // Create a new Web3 instance and assign it to a global variable
        ...
        window.ethereum
          ...
          .then((accounts) => {
            ...
            // Send the account address back to Unity
            unityInstanceGlobal.SendMessage('AuthenticateManager', 'Web3AddressReceived', account);
          })
          .catch((error) => {
            console.error("Error during account access request:", error);
            unityInstanceGlobal.SendMessage('AuthenticateManager', 'Web3AddressReceived', "Error");
          });
      } else {
  ...
  function _ModalSubscribeState(callbackPtr) {
          _appKitConfig.modal.subscribeState(newState => {
              ...
              (function(a1) {  dynCall_vi.apply(null, [callbackPtr, a1]); })(newStateStr);
              _free(newStateStr);
          });
      }

  function _SignTransaction(messagePtr) {
      if (typeof window.web3 !== 'undefined' && typeof window.ethereum !== 'undefined') {
        ...
        window.web3.eth.getAccounts()
          .then((accounts) => {
            ...
            // Use Web3.js to sign the message
            return window.web3.eth.personal.sign(message, account, "");
          })
          .then((signature) => {
            console.log("Message signed successfully:", signature);
            // Send the signature back to Unity
            unityInstanceGlobal.SendMessage('AuthenticateManager', 'Web3ASignReceived', signature);
          })
          .catch((error) => {
            console.error("Error signing message:", error);
            //unityInstanceGlobal.SendMessage('AuthenticateManager', 'Web3ASignReceived', "Error");
          });
      } else {
  ...
  function _WagmiWatchChainId(callbackPtr) {
          _appKitConfig.wagmiCore.watchChainId(_appKitConfig.config, {
              onChange(data) {
                  (function(a1) {  dynCall_vi.apply(null, [callbackPtr, a1]); })(data);
                  _free(dataStr);
              }
          });
      }
...
var asmLibraryArg = {
  ...
  "InitializeEthProvider": _InitializeEthProvider,
